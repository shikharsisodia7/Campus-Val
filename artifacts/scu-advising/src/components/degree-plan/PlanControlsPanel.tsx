...
/**
 * Everything that mutates which plan is active or edits its
 * majors/minors/professional-prep, deliberately kept OUT of the always-on
 * APR column (see ContextPanel.tsx) per the professor's correction that the
 * right-hand column must stay the official-record reference, never a
 * Progress/Plan toggle. Rendered inside a Sheet triggered from the Degree
 * Plan header.
 */
export function PlanControlsPanel({ plans }: { plans: AcademicPlan[] }) {
