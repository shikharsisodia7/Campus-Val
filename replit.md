...
- Pages: onboarding wizard, dashboard, courses (catalog browser with detail drawer; sections grouped by term with prominent professor/time/LAB-badge cards), planner (class-standing-aware unit caps), weekly schedule (visual M-F grid with real-time time-conflict detection, fed by "Add to Schedule" button on each section card; storage `campusval.schedule.v2`), GPA calc/sim, transfer (ASSIST.org articulation lookup), policies, AI advisor (SSE), graduation paths (3-yr & 4-yr CSE), AI evaluation (risk-weighted benchmark)
